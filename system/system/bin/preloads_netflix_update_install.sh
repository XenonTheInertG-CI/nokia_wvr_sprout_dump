...
if [ $# -eq 1 ]; then
  ...
  log -p i -t preloads_copy "install from $mountpoint"
  # Parallelize by copying subfolders and files in the background
  for file in $(find ${mountpoint} -type f); do
      log -p i -t preloads_copy "install $file"
      /system/bin/pm install $file
  done
  # Wait for jobs to finish
  # wait
  log -p i -t preloads_copy "install complete"
  exit 0
else
